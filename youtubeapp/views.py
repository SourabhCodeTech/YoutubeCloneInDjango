    ...
    if request.method == "POST":
        ...
        if cpass != password:
            ...
            if email and username and password:
                ...
                if a:
                    messages.success(
                        request, 'Please Login And You are now member of our family')
...
def login1(request):
    if request.method == "POST":
        ...
        if user:
            ...
            messages.warning(
                request, 'Please First Create Account or Account Details are wrong')
            return redirect('login')
...
def videoupload(request):
    ...
    if request.method == 'POST':
        ...
        if youtuber_video:
            video = Video.objects.create(youtuber_video=youtuber_video, video_thumbnail=video_thumbnail,
                                         video_title=video_title, video=video, video_desc=video_desc)
            if video:
...
def create_channel(request):
    if request.method == 'POST':
        ...
        youtuber = Youtuber.objects.filter(youtuber=request.user).update(
            channel_name=channelname, youtubeimage=file)
        if youtuber:
            return JsonResponse({
                'success': True,
                'link': 'channel/'
